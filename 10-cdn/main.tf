...
resource "aws_cloudfront_distribution" "web_cdn" {
  origin {
    domain_name              = "web-${var.environment}.${var.zone_name}" # web-dev.gopichand.online
  ...
  aliases = ["web-${var.common_tags.Component}.${var.zone_name}"] # web-cdn.gopichand.online
  ...
  default_cache_behavior {
    ...
    cache_policy_id        = data.aws_cloudfront_cache_policy.cdn_disable.id
  }
  ...
  ordered_cache_behavior {
    ...
    compress               = true
    viewer_protocol_policy = "redirect-to-https"
    cache_policy_id        = data.aws_cloudfront_cache_policy.cdn_enable.id
  }
...
module "records" {
  ...
  records = [
    {
      name    = "web-cdn" # web-cdn.gopichand.online
      type    = "A"
